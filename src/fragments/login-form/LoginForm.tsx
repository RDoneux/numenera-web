...
export default function LoginForm({ loginState, onSubmit }: LoginFormProps) {
  ...
  const isSubmitDisabled = useMemo(() => {
    const failedAndUnchanged =
      ...
      username === lastLoginAttempt.current.username &&
      password === lastLoginAttempt.current.password
    return (
      !username ||
      !password ||
  ...
  return (
    <form onSubmit={validateAndHandleSubmit}>
      <Stack
        spacing={2}
        className="w-[40vw] mx-auto background-contrast px-12 pt-5 pb-8 rounded-md border border-[#32383E]"
      >
        <NumeneraLogo />
        <Typography
          sx={{
